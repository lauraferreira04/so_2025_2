...
Ela mostra na tela a entrada e saída dos vários terminais (está configurada para 4), os prints para *debug* do simulador (feitos com a função `console_printf`), além da interface com o operador.
Essa interface permite realizar entrada e saída nos terminais e também controlar a simulação.

As funções que a console usa para acesso ao teclado e à tela estão em `tela_curses.c`, que implementa as funções descritas em `tela.h` usando a biblioteca "curses".
...
Além desses, tem um registrador de **erro**, para quando a CPU detecta algum problema, e um registrador de **complemento**, para quando o registrador de erro não é suficiente para codificar o problema.
Todos os registradores contêm um valor `int`, e são inicializados em 0.
...
- **D**, altera a velocidade da simulação (o valor default corresponde a D5; D0 é o mais rápido possível)
- **P**, para a execução, o controlador não vai mais mandar a CPU executar instruções em seu laço (o simulador inicia nesse modo)
...
Na forma como está o código, apesar da console controlar 4 terminais, só os dois primeiros estão sendo registrados no controlador de E/S na inicialização do hardware (em `main.c`): dispositivos 0, 1, 2 e 3 para o primeiro terminal e 4, 5, 6 e 7 para o segundo. Os dispositivos 16 e 17 estão sendo registrados como os dispositivos 0 e 1 do relógio.
